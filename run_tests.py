...
def _run_tests(exe, test_dir):
    ...
    for example in test_examples:
        ...
        test_passed = True
        mismatching_res = None
        for res in os.listdir("results"):
            if not res.endswith(".csv"):
                continue

            res_data = np.genfromtxt(os.path.join("results", res), delimiter=',', skip_header=3)
            ref_data = np.genfromtxt(os.path.join(test_dir_abs_path, example, "reference", res), delimiter=',', skip_header=3)
        ...
        if test_passed:
            ...
            print("Results {} do not match".format(mismatching_res))
            num_failed += 1
            print("\tFAIL")
